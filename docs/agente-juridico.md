...
Automatiza a rotina do jurídico interno: recebe pushes/e-mails/intimações,
consulta o andamento processual e, quando há providência a tomar, cadastra o
caso no Monday com prazo fatal e audiência.

## Fluxo

```
Gmail (intimações/pushes) ──► parser (nº CNJ, tipo, prazo, audiência, vara)
                                   │
                                   ▼
                          triagem de providência
                    (contestar, manifestar, audiência,
                     analisar recurso, tomar ciência)
                                   │
              ┌────────────────────┼──────────────────────┐
              ▼                    ▼                       ▼
      DataJud (CNJ)          Monday (board            eventos JSONL
   andamento processual      "processos": prazo     data/juridico-events.jsonl
      (opcional)             fatal + audiência)     (handoff p/ agentes futuros)
```
...
juridico process                   # triagem + Monday + eventos
...
## Andamento processual (DataJud)

A consulta usa a [API pública do DataJud/CNJ](https://datajud-wiki.cnj.jus.br/api-publica/).
Configure a chave (pública, publicada pelo CNJ) em `DATAJUD_API_KEY`. O alias do
tribunal é inferido do número CNJ (ex.: `tjsp`, `trf3`, `trt2`); use `--alias`
para forçar. Sem a chave, o `process` segue funcionando sem enriquecimento.
