...
def _pa_complaint() -> ProconComplaint:
    return ProconComplaint(
        ...
        response_deadline=date(2026, 7, 22),
        cause="Atraso na entrega",
        pdf_path="downloads/pa-test.pdf",
...
@patch("classificacao_procons.pipeline.fetch_complaint")
@patch("classificacao_procons.pipeline.save_complaint_pdf")
def test_should_process_administrative_process_notification(
    save_pdf_mock,
    fetch_complaint_mock,
    update_monday_mock,
    _has_token_mock,
    ...
    fetcher = MagicMock()
    fetcher_cls_mock.from_credentials.return_value = fetcher
    fetcher.list_unread_notifications.return_value = [_pa_notification()]
    fetch_complaint_mock.return_value = _pa_complaint()
    save_pdf_mock.return_value = DriveUploadResult(
        consumer_folder_id="folder-1",
        consumer_folder_url="https://drive.google.com/folder/folder-1",
        pdf_file_id="file-pa",
        pdf_url="https://drive.google.com/file/file-pa/view",
    )
    update_monday_mock.return_value = MondayRegistrationResult(
        item_id="monday-pa",
        board_id="board-1",
        item_url="https://b4a.monday.com/boards/board-1/pulses/monday-pa",
    )
    ...
    assert len(results) == 1
    assert results[0].status == "success"
    assert results[0].notification_type == "processo_administrativo"
    assert results[0].administrative_process_number == "35.001.003.26.1620383"
    assert results[0].monday_item_url.endswith("/monday-pa")
    fetch_complaint_mock.assert_called_once()
