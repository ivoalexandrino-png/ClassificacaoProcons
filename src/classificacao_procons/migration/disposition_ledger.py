...
para rastreabilidade. Não grava secrets.
"""
...
class DispositionLedgerEntry:
    ...
    #: Onda de migração (dimensão independente da disposição).
    wave: str | None = None
...
class DispositionLedger:
    ...
    entries: list[DispositionLedgerEntry] = field(default_factory=list)
    natives: list[SundayNativeEntry] = field(default_factory=list)

    def add(self, entry: DispositionLedgerEntry) -> None:
    ...
    def entry_for_monday(self, monday_item_id: str) -> DispositionLedgerEntry | None:
        for entry in self.entries:
