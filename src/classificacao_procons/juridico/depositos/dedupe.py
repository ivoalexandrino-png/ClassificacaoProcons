"""Deduplicação de guia + comprovante do mesmo pagamento."""
...
from classificacao_procons.juridico.depositos.path_rules import deposit_document_priority
...
def _record_key(record: JudicialDepositRecord) -> tuple[str, str, str]:
...
def _record_score(record: JudicialDepositRecord) -> tuple[int, int, int]:
    path = record.drive_path
    file_name = path.rsplit("/", 1)[-1]
    return (
        _CONFIDENCE_RANK.get(record.confidence, 0),
        1 if record.amount_brl is not None else 0,
        deposit_document_priority(drive_path=path, file_name=file_name),
    )


def dedupe_deposit_records(records: list[JudicialDepositRecord]) -> list[JudicialDepositRecord]:
    best_by_key: dict[tuple[str, str, str], JudicialDepositRecord] = {}
