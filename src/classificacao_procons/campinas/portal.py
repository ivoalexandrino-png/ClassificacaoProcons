"""Cliente do portal Procon Campinas via Playwright."""
...
class CampinasPortalError(RuntimeError):
    """Erro ao acessar ou extrair dados do Procon Campinas."""


@dataclass(frozen=True)
class CampinasPortalOptions:
...
def _fill_login_fields(page: Page, *, login: str, password: str) -> None:
    ...
    for selector in (
        ...
        if locator.count():
            ...
            break
    if not login_filled:
        raise CampinasPortalError("Campo de login não encontrado no portal Campinas.")

    password_input = page.locator("input[type='password']")
    if not password_input.count():
        raise CampinasPortalError("Campo de senha não encontrado no portal Campinas.")
...
def _open_protocol(page: Page, protocol_number: str) -> bool:
    for placeholder in ("Protocolo", "CIP", "Número", "Numero", "Pesquisar"):
...
def _extract_complaint_from_page(
    ...
    *,
    ...
    consumer_name = (
        labeled_value("Nome")
        or labeled_value("Nome completo")
        or consumer_name_hint
        or ""
    )
    consumer_cpf = _normalize_cpf(labeled_value("CPF") or consumer_cpf_hint or "")
    cause = labeled_value("Reclamação") or labeled_value("Assunto") or labeled_value("Descrição")
...
def _download_pdf_if_available(page: Page, download_dir: Path, protocol_number: str) -> str | None:
    ...
    for label in ("Imprimir", "PDF", "Baixar", "Download"):
        button = page.locator("button", has_text=label)
        ...
        try:
            ...
            target_path = download_dir / f"campinas-{safe_protocol}.pdf"
...
def fetch_campinas_complaint(options: CampinasPortalOptions) -> ProconComplaint:
    """Login no portal Campinas e abre a CIP pelo número."""
    options.download_dir.mkdir(parents=True, exist_ok=True)
    portal_url = options.credentials.portal_url
    if not portal_url:
        raise CampinasPortalError("URL do portal Campinas não configurada.")
    ...
    with sync_playwright() as playwright:
        ...
        try:
            ...
            if not _open_protocol(page, options.protocol_number):
                raise CampinasPortalError(
                    f"CIP {options.protocol_number} não encontrada no portal Campinas.",
                )
            ...
            raise CampinasPortalError(
                "Portal Campinas não respondeu a tempo durante o acesso.",
            ) from exc
        finally:
            browser.close()
